    ...
    <main className="min-h-screen bg-black text-white p-4 relative flex items-center justify-center">
     {showMosaic ? (
        ...
        <div className="grid grid-cols-3 md:grid-cols-6 lg:grid-cols-8 gap-2 w-full h-full align-top content-start">
          {images.map((img) => (
            <div key={img._id} className="aspect-square relative overflow-hidden bg-neutral-900 rounded-sm">
              <img
                src={img.url}
                alt="User upload"
      ...
      {showMosaic && (
          <div className="fixed top-6 right-6 z-50">
             <div className="bg-black/50 backdrop-blur px-4 py-2 rounded-full border border-white/10 flex items-center gap-2">
                <div className={`w-2 h-2 rounded-full animate-pulse ${engine === 'AI' ? 'bg-blue-500' : 'bg-pink-500'}`}></div>
                <span className="text-xs font-bold text-white tracking-widest">{engine} ENGINE ACTIVE</span>
             </div>
          </div>
      )}
