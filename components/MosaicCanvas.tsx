...
export default function MosaicCanvas({ imageUrls }: MosaicCanvasProps) {
  ...
  const generateMosaic = (loadedImages: HTMLImageElement[]) => {
    ...
    gradient.addColorStop(0, '#033541ff') // Pure black for better contrast
    ...
    const targetHeight = height * 0.9 // Leave some margin
    ...
    const centerY = height / 2 + fontSize * 0.02 // Reduced offset for larger font
  ...
  useEffect(() => {
    ...
    const intervalId = setInterval(() => {
      ...
      // Close after 1.7 seconds (total cycle ~2.9s fits in 3s interval)
      setTimeout(() => {
        setIsExpanded(false)
        setTimeout(() => {
          setActiveTile(null)
        }, 1200) // 1.2s transition back
      }, 1700)
    }, 3000) // Interval 3 seconds per user request
    ...
    >
      <div
        ...
        style={{
          ...
          cursor: 'crosshair',
          background: 'black', // Ensure black bg in full screen
          width: '100%',
        ...
        {activeTile && (
          <div
            style={{
              ...
              backgroundSize: 'contain',
              backgroundRepeat: 'no-repeat',
              backgroundColor: isExpanded ? 'rgba(0,0,0,0.9)' : 'rgba(0,0,0,0)',
              borderRadius: '12px',
